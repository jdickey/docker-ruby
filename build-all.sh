...
#    built with (currently Debian 'buster' or 'alpine-3.10').
...
#    Debian OS used for the upstream image (currently 'buster'; 'jessie' and
#    'stretch' are no longer supported by these images).
# 5. An image tag containing a _partial_ version number, such as '2-alpine'
#    or '2.5-buster', **must** be synonymous with the _latest_ matching version
...
for RUBY_VERSION in 2.6.5 2.5.7; do
  for BUSTER in buster slim-buster; do
    docker_build $RUBY_VERSION $BUSTER
  done

  for ALPINE_VERSION in 3.10; do
